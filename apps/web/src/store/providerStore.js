...
function mergePersistedProviders(persistedProviders) {
    ...
    const builtIns = BUILTIN_PROVIDERS.map((current) => {
        ...
        return {
            ...
            enabled: current.id === "ollama"
                ? true
                : persisted.enabled,
        };
    });
    const customProviders = persistedProviders.filter((provider) => !BUILTIN_PROVIDER_IDS.has(provider.id));
    return [...builtIns, ...customProviders];
}
export const useProviderStore = create()(persist((set, get) => ({
    ...
    setApiKey: (providerId, key) => {
        ...
        set((s) => ({
            ...
            providers: s.providers.map((p) => p.id === providerId ? { ...p, hasApiKey: key.length > 0 } : p),
        }));
    ...
    clearApiKey: (providerId) => {
        set((s) => {
            ...
            return {
                apiKeys: rest,
                providers: s.providers.map((p) => p.id === providerId ? { ...p, hasApiKey: false } : p),
            };
        });
        try {
