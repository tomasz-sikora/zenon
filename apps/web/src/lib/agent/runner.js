...
export async function runAgent(opts) {
    const { conversation, agent, selectedSkillIds, onChunk, onThinking, onToolCall, onToolResult, onComplete, onError, onRetry, signal, } = opts;
    const providerStore = useProviderStore.getState();
    const providerConfig = providerStore.providers.find((p) => p.id === agent.model.providerId);
    if (!providerConfig) {
    ...
    const requiresApiKey = providerConfig.type !== "openai-compatible" &&
    ...
    while (round < MAX_TOOL_ROUNDS) {
        ...
        for (const tc of pendingToolCalls) {
            ...
            if (count >= 3) {
                onError(new Error(`Infinite tool loop detected: "${tc.name}" called 3× with identical arguments. ` +
                    `Stopping to prevent runaway execution.`));
                return;
            }
        }
        const toolCallContent = pendingToolCalls.map((tc) => ({
            type: "tool_use",
            toolCallId: tc.id,
            toolName: tc.name,
            toolInput: tc.input,
